...
import { createStaff, getRoles, type HospitalRoleDto } from '@/lib/api';
...
export function CreateStaffModal({ branchId, onClose, onSuccess }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!hospitalRoleId) { toast.error('Please select a role.'); return; }
    setSaving(true);
    try {
      await createStaff(branchId, {
        firstName,
        lastName,
        hospitalRoleId,
        canLogin,
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6 space-y-4">
            ...
            <div>
              ...
              {roles.length === 0 ? (
                <div className="text-sm text-slate-500 py-2.5 px-3.5 bg-slate-800/50 border border-slate-700 rounded-lg">
                  No roles defined yet.{' '}
                  <a href="/dashboard/roles" className="text-teal-400 underline" target="_blank">Create a role first.</a>
                </div>
              ) : (
                <select
            ...
            <div className="flex items-center justify-between py-3 px-4 bg-slate-800/50 border border-slate-700 rounded-lg">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">Staff member can sign in to the system</p>
              </div>
              <button
                type="button"
            ...
            {canLogin && (
              <>
                <div>
                  ...
                  <input type="email" className={INPUT} value={email} onChange={e => setEmail(e.target.value)} placeholder="staff@hospital.com" required={canLogin} />
                </div>
                <div>
